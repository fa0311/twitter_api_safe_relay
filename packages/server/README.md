...
In the workspace, use `pnpm --filter twitter-api-safe-proxy dev:debug` during development. It serves `/api/events` and `/i/api/graphql/*` on port `3000` for the Vite dashboard UI.
